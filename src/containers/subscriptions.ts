...
export function bindSubscriptions(container: Container) {
  container
    .bind<DeleteRepliesOnCommentDeleted>(
      CONSTANTS.DeleteRepliesOnCommentDeleted,
    )
    .to(DeleteRepliesOnCommentDeleted);
  container
    .bind<DeleteCommentsOnPostDeleted>(CONSTANTS.DeleteCommentsOnPostDeleted)
    .to(DeleteCommentsOnPostDeleted);
  container
    .bind<NotifyPostAuthorOnPostLiked>(CONSTANTS.NotifyPostAuthorOnPostLiked)
    .to(NotifyPostAuthorOnPostLiked);
  container
    .bind<NotifyReceiverOnMessageCreated>(
      CONSTANTS.NotifyReceiverOnMessageCreated,
    )
    .to(NotifyReceiverOnMessageCreated);
  container
    .bind<NotifyFollowerOnPostCreated>(CONSTANTS.NotifyFollowerOnPostCreated)
    .to(NotifyFollowerOnPostCreated);
  container
    .bind<NotifyPostAuthorOnCommentCreated>(
      CONSTANTS.NotifyPostAuthorOnCommentCreated,
    )
    .to(NotifyPostAuthorOnCommentCreated);
  container
    .bind<SendMessageOnMessageCreated>(CONSTANTS.SendMessageOnMessageCreated)
    .to(SendMessageOnMessageCreated);
  container
    .bind<NotifyCommentAuthorOnCommentCreated>(
      CONSTANTS.NotifyCommentAuthorOnCommentCreated,
    )
    .to(NotifyCommentAuthorOnCommentCreated);
  container
    .bind<NotifyCommentAuthorOnCommentLiked>(
      CONSTANTS.NotifyCommentAuthorOnCommentLiked,
    )
    .to(NotifyCommentAuthorOnCommentLiked);
}
