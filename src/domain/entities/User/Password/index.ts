...
class Password extends ValueObject {
  ...
  public static create(passwordProps: PasswordProps): Result<Password, ValidationError> {
    ...
    if (passwordProps.value.length > 100) {
      return Err(new ValidationError("Password cannot exceed 100 characters"));
    }
    ...
    const hasSpecialChar = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(passwordProps.value);

    if (!(hasUppercase && hasLowercase && hasNumber && hasSpecialChar)) {
      return Err(new ValidationError("Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character"));
    }
  ...
  public async verify(plainPassword: string): Promise<Result<void, UnauthenticatedError>> {
