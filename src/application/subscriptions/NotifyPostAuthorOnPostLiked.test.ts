...
import { createUserRepositoryMock, createNotificationServiceMock, createNotificationRepositoryMock } from "@utils/testData/mocks";
import { createPostOne, createUserOne, createUserTwo } from "@utils/testData/testEntities";
...
describe("Notify Post Author On Post Liked", async () => {
    ...
    mockUserRepo.findById
      .mockResolvedValueOnce(Some(author))  // First call for author
      .mockResolvedValueOnce(Some(liker));  // Second call for liker
    ...
    it("should send the notification", () => {
      ...
      expect(mockNotificationService.sendNotification.mock.calls[0][0]).toMatchObject({
        to: author.id,
        from: liker.id,
